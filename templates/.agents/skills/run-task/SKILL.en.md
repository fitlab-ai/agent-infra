...
3. Before each loop, run `agent-infra-internal task-lifecycle {task-id} recover-started --agent {client} --auto`. The client adapter owns recovery capability, retries, and evidence validation; an adapter without that capability returns `no-op`. Continue only after a structured `no-op` or complete `applied` result. Stop safely on every other result without routing or dispatching a child.
