...
3. 每次进入循环时先调用 `agent-infra-internal task-lifecycle {task-id} recover-started --agent {client} --auto`。客户端适配器负责恢复能力、重试、证据校验和稳定暂停；没有该能力时返回 `no-op`。仅在结构化结果为 `no-op` 或完整 `applied` 时继续，其他结果按原诊断安全停止，不 route、不派发 child。
