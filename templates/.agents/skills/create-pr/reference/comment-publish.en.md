...
- Populate the "Manual Verification Required" section per the aggregation rules in `.agents/rules/pr-sync.md`: include only post-code-stage checks that the AI cannot close on its own and that require a human to execute or judge; sources are `review-code*` "Environment-Blocked Findings" plus `code*` items that genuinely require a real environment, permissions, account, external system, or human judgment to confirm the effect; exclude `analysis*`/`plan*` "Assumptions"/"Open Questions", directional leftovers from `review-plan*`, `review-*` "Self-Doubt", and items an agent can close on its own; each item must state "what to verify + location + why only a human can verify it"; write the explicit placeholder when there are none
