...
Before changing a title, run `agent-infra-internal platform-context resolve` for upstream and capabilities, then follow the Issue metadata intent contract in `.agents/rules/issue-pr-commands.md`; if permission is unavailable, skip the change and inform the user.

### 5. Inform User

> Before rendering next steps, read `.agents/rules/next-step-output.md`; when a next-step client command is needed, invoke the shared helper for the task's current stage.
