...
- A controlled candidate's `title`, `description`, and all seven task-input lists must already be safe Markdown when generated: format an explicit non-user token such as `@2x` as code (`` `@2x` ``), while preserving semantically meaningful `@` forms such as email addresses, real user mentions, and URLs. This applies only to candidates generated by this skill; handwritten or external content is not rewritten.
