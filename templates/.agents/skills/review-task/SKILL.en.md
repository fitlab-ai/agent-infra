...
Include all TUI command formats in the next-step output. If `.agents/.airc.json` configures `customTools`, read each tool's `name` and `invoke`, then add the matching command line in the same format (`{name}` becomes the skill name and `&#123;project&#125;` becomes the project name).
...
- [ ] Informed the user of the next step (must include all TUI command formats, including customTools; do not filter)
