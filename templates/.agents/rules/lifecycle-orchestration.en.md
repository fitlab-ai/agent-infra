...
- claude-code's requested reasoning effort does not yet support per-role dispatch (concurrent tasks would race on a shared file, see HDR-8); `delegationEvidence.actualReasoningEffort` is declared `spawn-ack`, meaning it is only recorded when honestly observed in a native spawn lifecycle event (Start/Stop) — it is not a promise of per-role dispatch and does not gate activation.
