...
- 需人工校验段落：按「先过滤、再写清、最后补源」（③→②→①）从各阶段最新产物聚合，避免无差别搬运放大噪音。
  - **候选来源（①）**：提取「需人工确认/兜底」性质的条目——`analysis*` 的「假设」「未决问题」、`plan*` 的「假设」「未决问题」、`review-plan*` 的「环境性遗留」「自我质疑」、`code*` 的「供审查关注的内容」「已知问题」、`review-code*` 的「环境性遗留」「自我质疑」。
  - **纳入判据（③，三点须同时满足，否则排除）**：(a) 改动落在本 PR diff 内（排除 `task.md` 等 workspace 内部簿记）；(b) 结论尚未被批准、确需人判断（排除已过 review-plan/review-code 的决策，以及对外部参考的免责式说明）；(c) 有明确定位（文件/改动/范围）。
  - **写法（②，最小信息量）**：保留的每条至少写明「校验什么 + 定位（文件/改动/范围）+ 为何需人工」，禁止只原样搬运一句「假设 X」。
  - **占位**：过滤后无任何条目时，写显式占位 `- 无需人工校验事项`，不得留空。
