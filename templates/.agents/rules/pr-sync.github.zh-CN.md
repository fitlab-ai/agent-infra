...
- 需人工校验段落：只收进入 code 阶段后仍需人实际执行或判断、AI 无法自行关闭的校验点；方向性/决策类问题属于阶段间人工审查检查点，进入 code 前应已拍板，不进入本段。
  - **候选来源**：`review-code*` 的「环境性遗留」，以及 `code*` 中确需真实环境、权限、账号、外部系统或人工判断才能确认效果的校验点。
  - **排除**：`analysis*`/`plan*` 的「假设」「未决问题」、`review-plan*` 的方向性遗留、`review-*` 的「自我质疑」，以及能由 agent 通过重跑测试、补充检查或下一轮修复关闭的事项。
