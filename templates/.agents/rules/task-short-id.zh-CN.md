...
短号让所有 SKILL 在 active 任务生命周期内可以用裸数字 `N` 替代
完整的 22 字符 `TASK-YYYYMMDD-HHMMSS`。

## 语法

- 只接受**裸数字 `N`**（无需 shell 引号）：如 `1`、`7`、`42`。
...
- 完整 `TASK-…` 入参在所有路径下行为与现状等价；裸数字只是别名，不是持久化任务 ID。
- 已移除的 `#N` / `#NN` 语法作为无效任务引用稳定拒绝。
...
- **保留键**：解析后 `n == 0`（输入如 `0`、`00`）。退出码 1。
- **超容量**：解析后 `n > 10^shortIdLength - 1`（如 `shortIdLength=2` 下 `100`）。退出码 1。
- **参数格式错误**：入参既不是 `^[0-9]+$` 也不是 `TASK-id`（如 `#1`、`#abc`、`#`、`5.5`）。退出码 1。
