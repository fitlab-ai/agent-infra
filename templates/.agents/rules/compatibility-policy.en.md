...
4. **Exit**: removal conditions, ownership, and the code, tests, and documentation to delete.
...
- **Implementation**: implement only the approved compatibility budget. Do not preserve an unapproved old branch “just in case.”
- **Review**: verify the four admission items, single source of truth, and exit condition. Flag unauthorized compatibility, indefinite shims, dual writes, and runtime-path migrations as scope and maintainability problems.

## Existing Compatibility Logic

During an ordinary task, record out-of-scope compatibility logic and its risk without deleting it. A dedicated simplification task should first inventory consumers and stored data, then process items in this order: remove now, warn then remove, retain temporarily.
