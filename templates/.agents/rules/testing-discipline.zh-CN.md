...
请为本仓库填写以下各项，记录选定行为及其边界，以便贡献者一致地选择并验证测试。

如果项目使用跨平台 smoke 边界，应将真实 CLI、git、shell、环境透传和进程生命周期测试放在 unit 层之外；记录该边界运行的操作系统，并将测试层级结构检查与代码行覆盖率分开。

### 1. 测试命令

> TODO: 记录完整测试命令，以及开发期间使用的可选快速命令。

### 2. 测试目录或层级

> TODO: 记录测试目录或层级，以及选择对应目录或层级的规则。

### 3. 覆盖率

> TODO: 记录覆盖率采集方式、阈值，以及覆盖率是否阻塞变更。

### 4. 持续集成

> TODO: 记录运行测试的 CI 检查，以及哪些检查属于阻塞项或信息项。

### 5. 报告服务

> TODO: 记录外部报告服务、徽章或构件，以及它们是否影响验收。
