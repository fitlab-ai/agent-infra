...
- `review-code` records the `审查基线提交` (`git rev-parse HEAD`) in its report.
- After a `review-code` Approved verdict, if new commits touch code / rule paths, the `post-review-commit` gate in `complete-task` blocks and requires a fresh `review-code`.
