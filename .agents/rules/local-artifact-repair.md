...
4. 不涉及人工裁决、任务状态、账本状态、receipt、Git、平台或其他外部副作用。
...
6. 模型无法证明下一步安全，或判断问题属于环境、权限、并发、身份、provenance、状态未知、人工裁决或其他非本地问题时，立即停止。
...
- 只有最终一次完整 finalizer 成功，且同一次结果的 provenance、账本状态和 `stageStatus.canAdvance` 允许推进时，才能发布 review completed 事件和跨阶段 next-step 命令。
