...
- 按 `.agents/rules/pr-sync.md` 的聚合规则填充「需人工校验」段落：只收进入 code 阶段后 AI 无法自行关闭、需要人实际执行或判断的校验点，来源为 `review-code*` 的「环境性遗留」以及 `code*` 中确需真实环境/权限/账号/外部系统/人工判断才能确认效果的点；排除 `analysis*`/`plan*` 的「假设」「未决问题」、`review-plan*` 的方向性遗留、`review-*` 的「自我质疑」以及 agent 可自行关闭的事项；每条写明「校验什么 + 定位 + 为什么只能由人校验」；无事项时写显式占位
