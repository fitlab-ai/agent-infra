...
3. Codex 在每次进入循环时先调用 `agent-infra-internal task-lifecycle {task-id} recover-started --agent codex --auto`。`no-op` 或完整 `applied` 才继续；`RECOVERY_RELEASE_RETRY_REQUIRED` 原样重试一次。claim 已持有后的日志写入、提交复核、claim 释放或 run 保存等可重试事务错误，统一以 `ORCHESTRATION_LIFECYCLE_RECOVERY_INCOMPLETE`、底层 code/message 和 `recoverable=true` 调用 orchestration `pause`，完成 `run-task.paused` 校验后停止。其他证据缺失、异常终态、身份冲突、候选歧义或响应无法重建均按原诊断安全暂停，不 route、不派发 child。非 Codex client 跳过此 Codex-only intent，保持既有失败关闭边界。
...
6. 调用 `prepare --client {client} --requested-model {requestedModel} --requested-reasoning-effort {requestedReasoningEffort}`，使用 route 返回的精确策略。Codex 执行当前宿主 preflight；任务身份、策略或宿主检查失败时记录实际诊断。
...
8. Codex 通过 SubagentStart/Stop 或 parent spawn/wait 与 App Server 记录实际启动及终态。timed-out wait 不推断成功；身份、传输或异常终态失败如实暂停。阶段结果和 child 完成后调用 `advance`；只有 `running` 才重复步骤 3。
