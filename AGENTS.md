...
本节适用于面向人的中文项目文档和 AI 的中文回复；不适用于代码、命令、日志、引用内容、API 名称和已定义的专业术语。英文文档不机械翻译本节规则。

- 句子应说明明确的主体、动作、对象和必要条件。信息不足时，直接说明缺少的信息，不用套话填充篇幅。
- 只在确有因果、转折或递进关系时使用“但是”“因此”“同时”等连词，不为连接句子而添加连词。
- 不使用比喻、拟人、肢体动作式表达描述抽象概念；已经定义的专业用语不受此限。
- 不使用下列夸大或空泛的职场用语：赋能、抓手、颗粒度、沉淀、打法、势能、生态位、顶层设计、全链路、底层逻辑、拉齐、打通、对标、勾兑、倒逼、引爆点。
- 不使用无指向的代词或名词：东西、那一套、一个形状、这种感觉、那么回事。改用实际对象，例如产物、改动、材料、问题、来源、配置或数据。
- 不使用对象不明的万能动词，例如“走”“打”。改用执行、调用、创建、发送、读取、写入、比较或删除等具体动词。
- “不是 A 而是 B”仅在 A 有明确出处且该对比有助于理解时使用。
- 优先使用简短的现代技术书面语；不模仿文言文，不使用网络口语，不用总结性口号替代事实。
