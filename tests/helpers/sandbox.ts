...
function writeSandboxEngineFixture(
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "const fixtureContainerId = 'f'.repeat(64);",
      ...
      "  process.exit(0);",
      "}",
      "if (args[0] === 'rm' && args.at(-1)) {",
      "  const container = args.at(-1);",
      "  if (process.env.DOCKER_EXIT_FOR_RM_ID && process.env.DOCKER_EXIT_FOR_RM_ID === container) {",
      "    process.exit(Number(process.env.DOCKER_EXIT_FOR_RM_ID_CODE || '1'));",
      "  }",
      "  const remaining = dockerRows().filter((row) => row.split('\\t')[0] !== container);",
      "  fs.writeFileSync(dockerStatePath, remaining.join('\\n'), 'utf8');",
      "  if (process.env.DOCKER_REMOVAL_UPDATES_INSPECT === '1') rememberRemoved(container);",
      ...
      "  const id = args.at(-1) || '';",
      "  if (process.env.DOCKER_INSPECT_NOT_FOUND === '1' || removedContainers().has(id)) { process.stderr.write('No such container\\n'); process.exit(1); }",
      "  process.stdout.write(`${JSON.stringify({ Id: id, State: { Running: true }, Config: { Labels: inspectLabels(id) } })}\\n`);",
      "  process.exit(0);",
      "}",
      "if (args[0] === 'inspect') {",
      ...
      "  }",
      "  const formatIndex = args.indexOf('--format');",
      "  if (formatIndex >= 0 && args[formatIndex + 1] === '{{.Id}}') {",
      "    const configuredIds = process.env.DOCKER_INSPECT_IDS ? JSON.parse(process.env.DOCKER_INSPECT_IDS) : {};",
      "    process.stdout.write(`${configuredIds[args.at(-1)] || 'f'.repeat(64)}\\n`);",
      "    process.exit(0);",
      "  }",
      ...
      "    : [{ Id: (process.env.DOCKER_INSPECT_IDS ? JSON.parse(process.env.DOCKER_INSPECT_IDS)[args.at(-1)] : null) || fixtureContainerId, State: { Running: true }, Config: { Labels: inspectLabels(args.at(-1)) }, Mounts: process.env.DOCKER_INSPECT_NO_MOUNTS === '1' ? [] : inspectMounts() }];",
      ...
      "if (args[0] === 'version') {",
      "  const formatIndex = args.indexOf('--format');",
      "  if (formatIndex >= 0 && args[formatIndex + 1] === '{{json .Server}}') {",
      "    process.stdout.write(JSON.stringify({ Version: '28.2.0', ApiVersion: '1.50' }) + '\\n');",
      "    process.exit(0);",
      "  }",
      ...
      "}",
      "if (args[0] === 'exec' && args.at(-1) === 'npm ci' && process.env.DOCKER_EXIT_FOR_INIT_COMMAND) {",
      "  process.stderr.write('fixture project initialization failed\\n');",
      "  process.exit(Number(process.env.DOCKER_EXIT_FOR_INIT_COMMAND));",
      "}",
      "if (args[0] === 'info' && process.env.DOCKER_EXIT_FOR_INFO) {",
      "  process.exit(Number(process.env.DOCKER_EXIT_FOR_INFO));",
      "}",
      "if (args[0] === 'info' && args.at(-1) === '{{json .ID}}') {",
      "  process.stdout.write(JSON.stringify('fixture-daemon-id') + '\\n');",
      "  process.exit(0);",
      "}",
