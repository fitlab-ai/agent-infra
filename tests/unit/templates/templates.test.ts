...
test("human-decision context rules define complete comparable option blocks", () => {
  for (const relativePath of [
    ...
    assert.equal(summaryFields.length, 4, `${relativePath} should define four decision-context fields`);
    assert.equal(new Set(summaryFields.map((match) => match[1])).size, 4, `${relativePath} summary labels should be unique`);
    ...
    optionHeadings.forEach((option, index) => {
      ...
      assert.equal(labels.length, 5, `${relativePath} option ${index + 1} should define five comparison fields`);
      assert.equal(new Set(labels).size, 5, `${relativePath} option ${index + 1} labels should be unique`);
      optionLabels ??= labels;
