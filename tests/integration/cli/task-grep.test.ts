...
test('matches a literal pattern and prints taskId, short id, file stem and line number', () => {
  ...
  const out = runCli(['task', 'grep', 'needle', '--task', taskId], repoRoot);
...
test('no scope fails closed without a unique current task context', () => {
  const { repoRoot } = mkFixture();
...
test('-i / --ignore-case toggles case sensitivity', () => {
  ...
  const sensitive = runCli(['task', 'grep', 'NEEDLE', '--task', taskId], repoRoot);
  assert.equal(sensitive.status, 1);
  assert.equal(sensitive.stdout, '');

  // -i matches.
  const ignore = runCli(['task', 'grep', '-i', 'NEEDLE', '--task', taskId], repoRoot);
  ...
  const ignoreLong = runCli(['task', 'grep', '--ignore-case', 'NEEDLE', '--task', taskId], repoRoot);
...
test('pattern is matched literally, not as a regex', () => {
  ...
  const dot = runCli(['task', 'grep', 'a.c', '--task', taskId], repoRoot);
  ...
  const bracket = runCli(['task', 'grep', '[x]', '--task', taskId], repoRoot);
  ...
  const star = runCli(['task', 'grep', 'a*c', '--task', taskId], repoRoot);
...
test('a ref narrows the scan to a single task', () => {
  ...
  const out = runCli(['task', 'grep', 'KEYWORD', '--task', a], repoRoot);
  assert.equal(out.status, 0, out.stderr);
...
test('an artifact selector (name or number) narrows the scan to a single file', () => {
  ...
  const byName = runCli(['task', 'grep', 'KEYWORD', '--task', taskId, 'analysis'], repoRoot);
  ...
  const byNumber = runCli(['task', 'grep', 'KEYWORD', '--task', taskId, '2'], repoRoot);
...
test('no match exits 1 with empty stdout', () => {
  ...
  const out = runCli(['task', 'grep', 'does-not-exist-zzz', '--task', taskId], repoRoot);
  assert.equal(out.status, 1);
  assert.equal(out.stdout, '');
});

test('an omitted scope does not scan the full tree or reach archived tasks', () => {
  const { repoRoot } = mkFixture();
...
test('a TASK-id ref does reach an archived task', () => {
  ...
  const out = runCli(['task', 'grep', 'ARCHKEY', '--task', archived], repoRoot);
  assert.equal(out.status, 0, out.stderr);
...
test('-- ends option parsing so a pattern can start with a dash', () => {
  ...
  const out = runCli(['task', 'grep', '--task', taskId, '--', '-i'], repoRoot);
  assert.equal(out.status, 0, out.stderr);
