...
test("required template files were migrated into templates/", () => {
  const requiredFiles = [
    ...
    "templates/.agents/rules/sync-content-generation.en.md",
...
test("human-decision context rules define complete comparable option blocks", () => {
  for (const relativePath of [
    ...
    assert.ok(summaryFields.length >= 4, `${relativePath} should define the required decision-context fields`);
    assert.equal(new Set(summaryFields.map((match) => match[1])).size, summaryFields.length, `${relativePath} summary labels should be unique`);
    ...
    optionHeadings.forEach((option, index) => {
      ...
      const labels = fields.map((match) => match[1]!).filter((label) => !['建议选哪个', '为什么这样建议', 'Recommended choice', 'Why this choice'].includes(label));
      assert.ok(labels.length >= 5, `${relativePath} option ${index + 1} should define the required comparison fields`);
      assert.equal(new Set(labels).size, labels.length, `${relativePath} option ${index + 1} labels should be unique`);
      optionLabels ??= labels;
...
test("update-agent-infra template copies stay in sync with working files", () => {
  ...
  const syncFiles: Array<[string, string]> = [
    ...
    [".agents/rules/local-artifact-repair.md", "templates/.agents/rules/local-artifact-repair.en.md"],
    [".agents/rules/task-management.md", "templates/.agents/rules/task-management.en.md"],
    [".agents/rules/sync-content-generation.md", "templates/.agents/rules/sync-content-generation.en.md"],
