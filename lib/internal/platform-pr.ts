...
const USAGE = `Usage: agent-infra-internal platform-pr inspect <task-ref> [--cwd <path>]
       ...
       agent-infra-internal platform-pr summary-sync <task-ref> --agent <agent> --body-file <path|-> --change-report-file <path> --result <pr_created|pr_reused|no_op> [--manual-phase pending|final --evidence-file <path> ...] [--strict] [--dry-run] [--cwd <path>]
`;

const BOOLEAN_FLAGS = new Set(['--draft', '--dry-run', '--metadata', '--closing-issue', '--strict']);
const VALUE_FLAGS = new Set(['--cwd', '--agent', '--base', '--head', '--title-file', '--body-file', '--change-report-file', '--mechanical-file', '--precheck-file', '--pr', '--result', '--manual-phase', '--evidence-file', '--transaction-id', '--receipt-digest', '--evidence-digest', '--pr-head-sha']);
...
async function platformPr(args: string[] = []): Promise<void> {
  ...
  const allowed: Record<string, string[]> = {
    ...
    'summary-sync': ['cwd', 'agent', 'bodyFile', 'changeReportFile', 'result', 'manualPhase', 'evidenceFile', 'transactionId', 'receiptDigest', 'evidenceDigest', 'prHeadSha', 'dryRun', 'strict']
  };
  ...
  if (internalHandlerRoute('platform-pr', 'summary-sync', operation)) {
    ...
    try {
      ...
      const evidenceFile = values.evidenceFile;
      if (manualPhase !== undefined && manualPhase !== 'pending' && manualPhase !== 'final') { fail('--manual-phase must be pending or final'); return; }
      if (manualPhase && typeof evidenceFile !== 'string') { fail('manual summary sync requires --evidence-file'); return; }
      finish(await syncPullRequestSummary(taskRef, {
        ...
        ...(manualPhase ? { manualValidation: {
          phase: manualPhase,
          evidenceFile: evidenceFile as string,
          ...
          ...(typeof values.evidenceDigest === 'string' ? { evidenceDigest: values.evidenceDigest } : {}),
