...
const USAGE = `Usage: agent-infra-internal manual-validation transaction <task-ref> --prepare --artifact <artifact> --summary-file <path> --agent <agent> [--cwd <path>]
       agent-infra-internal manual-validation transaction <task-ref> --artifact <artifact> --summary-file <path> --change-report-file <path> --agent <agent> [--result pr_created|pr_reused|no_op] [--cwd <path>]`;
...
function printFailure(format: OutputFormat, error: { code: string; message: string }): void {
  if (format === 'json') process.stdout.write(`${JSON.stringify(result('failed', error))}\n`);
  else process.stderr.write(`${error.code}: ${error.message}\n`);
  process.exitCode = 1;
}

const MANUAL_VALIDATION_STATUS_HEADING = /^###\s+(?:⚠️\s+(?:需人工校验|Manual Validation Required)|✅\s+(?:人工验证已通过|无需人工校验|Manual Validation Passed|No Manual Validation Required)|⏳\s+(?:人工验证待收尾|Manual Validation Pending))\s*$/mu;

function manualSummaryBody(body: string, phase: 'pending' | 'final'): string {
  ...
  const marker = phase === 'final'
    ...
    : '';
  if (MANUAL_VALIDATION_STATUS_HEADING.test(body)) {
    return `${body.replace(MANUAL_VALIDATION_STATUS_HEADING, `${heading}${marker}`).replace(/\s+$/u, '')}\n`;
  }
  const fallback = phase === 'pending'
    ? chinese ? '人工验证正在等待最终完成。' : 'Manual validation is awaiting final promotion.'
    : chinese ? '人工验证已通过。' : 'Manual validation passed.';
...
async function executeManualValidationTransaction(taskRef: string, values: Record<string, string>, cwd: string, options: ManualValidationCoordinatorOptions = {}) {
  ...
  const required = prepareOnly
    ? ['artifact', 'summaryFile', 'agent'] as const
    : ['artifact', 'summaryFile', 'changeReportFile', 'agent'] as const;
...
async function executeManualValidationTransactionLocked(
  ...
  const summaryPath = path.resolve(cwd, values.summaryFile!);
  const finalBodyWithoutReceipt = fs.readFileSync(summaryPath, 'utf8');
  const evidenceDigest = sha256File(summaryPath);
  ...
  const pendingBody = manualSummaryBody(finalBodyWithoutReceipt, 'pending');
  const createTransaction = (): ManualValidationTransaction => createManualValidationTransaction({
    ...
    finalSummaryDigest: manualValidationFinalSummaryDigest(manualSummaryBody(finalBodyWithoutReceipt, 'final')),
  ...
  try {
    ...
    if (transaction.phase === 'receipt-committed' && !transaction.eventAppended) {
      const completed = applyTaskEvent({ taskRef, event: 'manual-validation.completed', agent, initiator: 'model', requestId: transaction.transactionId, reasonCode: 'user-request', artifact: values.artifact, summaryResult: 'human-confirmed validation and committed receipt', transactionId: transaction.transactionId, receiptDigest: receipt.receiptDigest, prHeadSha: receipt.prHeadSha }, { lockAlreadyHeld: true, repoRoot: cwd });
    ...
    const promoted = await syncPullRequestSummary(taskRef, {
      cwd, client: options.client, agent, body: manualSummaryBody(finalBodyWithoutReceipt, 'final'), changeReportFile: path.resolve(cwd, values.changeReportFile!), primaryResult, strict: true,
      manualValidation: { phase: 'final', transactionId: transaction.transactionId, receiptDigest: receipt.receiptDigest, prHeadSha: receipt.prHeadSha, authority: 'coordinator' }, lockAlreadyHeld: true
    });
...
function parse(args: string[], start: number): { values: Record<string, string>; error?: string } {
  ...
  for (let index = start; index < args.length; index += 1) {
    ...
    if (!['--format', '--cwd', '--artifact', '--summary-file', '--change-report-file', '--agent', '--result'].includes(flag)) return { values, error: `unknown option '${flag}'` };
...
async function manualValidation(args: string[] = []): Promise<void> {
  ...
  if (!isTransaction) { printFailure('json', { code: 'MANUAL_VALIDATION_ARGS_INVALID', message: 'transaction operation is required' }); return; }
...
export { executeManualValidationTransaction, manualValidation };
