 ...
 * @property {string[]=} legacySandboxBases
...
function createBuiltinTools(home, project) {
  ...
  return {
    'claude-code': {
      ...
      legacySandboxBases: [path.join(home, '.claude-sandboxes')],
    ...
    codex: {
      ...
      legacySandboxBases: [path.join(home, '.codex-sandboxes')],
    ...
    opencode: {
      ...
      legacySandboxBases: [path.join(home, '.opencode-sandboxes')],
    ...
    'gemini-cli': {
      ...
      legacySandboxBases: [path.join(home, '.gemini-sandboxes')],
...
export function toolConfigDirCandidates(tool, project, branch) {
  const bases = [tool.sandboxBase, ...(tool.legacySandboxBases ?? [])];
  return bases.flatMap((base) => safeNameCandidates(branch).map((name) => path.join(base, project, name)));
}

export function toolProjectDirCandidates(tool, project) {
  return [tool.sandboxBase, ...(tool.legacySandboxBases ?? [])]
    .map((base) => path.join(base, project));
}
