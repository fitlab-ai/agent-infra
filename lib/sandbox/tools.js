...
function createBuiltinTools(home, project) {
  ...
  return {
    'claude-code': {
      ...
      sandboxBase: joinHome(home, '.agent-infra', 'sandboxes', 'claude-code'),
      ...
      hostPreSeedDirs: [
        { hostDir: joinHome(home, '.claude', 'plugins'), sandboxSubdir: 'plugins' }
      ],
      pathRewriteFiles: [
      ...
      hostLiveMounts: [
        {
          hostPath: joinHome(home, '.agent-infra', 'credentials', project, 'claude-code', '.credentials.json'),
    ...
    codex: {
      ...
      sandboxBase: joinHome(home, '.agent-infra', 'sandboxes', 'codex'),
      ...
      hostLiveMounts: [
        { hostPath: joinHome(home, '.codex', 'auth.json'), containerSubpath: 'auth.json' }
      ],
      postSetupCmds: [
    ...
    opencode: {
      ...
      sandboxBase: joinHome(home, '.agent-infra', 'sandboxes', 'opencode'),
      ...
      hostLiveMounts: [
        {
          hostPath: joinHome(home, '.local', 'share', 'opencode', 'auth.json'),
    ...
    'gemini-cli': {
      ...
      sandboxBase: joinHome(home, '.agent-infra', 'sandboxes', 'gemini-cli'),
      containerMount: '/home/devuser/.gemini',
      versionCmd: 'gemini --version',
      setupHint: 'Run gemini inside the container to finish authentication.',
      hostLiveMounts: [
        { hostPath: joinHome(home, '.gemini', 'oauth_creds.json'), containerSubpath: 'oauth_creds.json' }
      ],
      hostPreSeedFiles: [
        { hostPath: joinHome(home, '.gemini', 'settings.json'), sandboxName: 'settings.json' },
        { hostPath: joinHome(home, '.gemini', 'google_accounts.json'), sandboxName: 'google_accounts.json' }
      ]
    }
  };
}
...
export function toolConfigDir(tool, project, branch) {
  return tool.sandboxBase.startsWith('/')
    ? path.posix.join(tool.sandboxBase, project, sanitizeBranchName(branch))
    : path.join(tool.sandboxBase, project, sanitizeBranchName(branch));
}

export function toolConfigDirCandidates(tool, project, branch) {
  return safeNameCandidates(branch).map((name) => (
    tool.sandboxBase.startsWith('/')
      ? path.posix.join(tool.sandboxBase, project, name)
      : path.join(tool.sandboxBase, project, name)
  ));
}
...
export function toolProjectDirCandidates(tool, project) {
  return [
    tool.sandboxBase.startsWith('/')
      ? path.posix.join(tool.sandboxBase, project)
      : path.join(tool.sandboxBase, project)
  ];
}
