    ...
    await p.tasks([
      ...
      },
      {
        ...
        task: async (message) => {
          ...
          const gnupgDir = path.join(effectiveConfig.home, '.gnupg');
          const gpgVolumes = (needsGpg && fs.existsSync(gnupgDir))
            ? ['-v', `${gnupgDir}:${CONTAINER_HOME}/.gnupg`]
            : [];
          const gpgMounted = gpgVolumes.length > 0;
          ...
          syncGitConfig(container, effectiveConfig.repoRoot, effectiveConfig.home, { gpgMounted });
