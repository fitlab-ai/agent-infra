...
export function hostShellConfigDir(home, project, branch) {
  return joinHome(home, '.agent-infra', 'config', project, sanitizeBranchName(branch));
}
...
export function writeSanitizedGitconfig({ home, hostConfigDir, stripGpg, repoRoot }) {
  const gitconfigPath = joinHome(home, '.gitconfig');
...
export function prepareHostShellConfig({ home, project, branch, repoRoot }) {
  ...
  for (const file of ['.gitignore_global', '.stCommitMsg']) {
    const hostPath = joinHome(home, file);
...
function gpgCacheDir(home, project) {
  return joinHome(home, '.agent-infra', 'gpg-cache', project);
}
...
export function extractClaudeCredentialsBlob(home, execFn = execFileSync) {
  ...
  const credentialsPath = joinHome(home, '.claude', '.credentials.json');
...
export function claudeCredentialsDir(home, project) {
  return joinHome(home, '.agent-infra', 'credentials', project, 'claude-code');
}

export function claudeCredentialsPath(home, project) {
  return joinHome(claudeCredentialsDir(home, project), '.credentials.json');
}
...
export function sandboxAliasesPath(home) {
  return joinHome(home, '.agent-infra', 'aliases', 'sandbox.sh');
}
        ...
        task: async () => {
          for (const { tool, dir } of resolvedTools) {
            ...
            for (const relativePath of tool.pathRewriteFiles ?? []) {
              ...
              content = content.replaceAll(effectiveConfig.home, path.posix.dirname(tool.containerMount));
      ...
      },
      {
        ...
        task: async (message) => {
          ...
          runEngineTaskCommand(engine, 'docker', [
            ...
            '-v',
            volumeArg(engine, joinHome(effectiveConfig.home, '.ssh'), '/home/devuser/.ssh', ':ro'),
            ...toolVolumes,
            ...liveMountVolumes,
            ...shellConfigVolumes,
            ...envArgs,
