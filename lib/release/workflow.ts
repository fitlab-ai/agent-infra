...
type ReleaseFacts = { localTag: boolean; localTagConflict?: boolean; remoteBranch: boolean; remoteTag: boolean; githubRelease: boolean | null; npm: boolean | null; homebrew: boolean | null; smoke: 'success' | 'pending' | 'failed' | null; postCommit: boolean };

function deriveReleasePhase(facts: ReleaseFacts): ReleasePhase {
  if (!facts.localTag) return 'unprepared';
...
async function runReleaseAction(input: { version: string; action: 'inspect' | 'prepare' | 'publish' | 'post'; inspect: () => Promise<ReleaseFacts>; prepare?: () => Promise<void>; publish?: () => Promise<void>; post?: () => Promise<void> }) {
  ...
  const allowed = input.action === 'prepare' ? before.phase === 'unprepared'
    : input.action === 'publish' ? ['prepared', 'partially-published'].includes(before.phase)
