...
function trustedTaskControlLauncherPrefix(): string {
  return [
    ...
    '  exit 1',
    'fi',
    'if [ "$task_control_command" != true ]; then launcher_proof_required=false; fi',
    'if [ "$status_mount_present" = true ] && [ -n "$launch_config" ]; then',
    '  launcher_proof_required=false',
    'fi',
    'if [ "$launcher_proof_required" = true ] && [ ! -f "$launcher_auth_path" ]; then',
    '  umask 077',
    '  mkdir -p "$launcher_auth_dir"',
    '  od -An -N32 -tx1 /dev/urandom | tr -d \'[:space:]\' > "$launcher_auth_path"',
    'fi',
    'if [ "$launcher_proof_required" = true ]; then launcher_key=$(cat "$launcher_auth_path"); fi',
    'if [ "$launcher_proof_required" = true ] && [ "${#launcher_key}" -ne 64 ]; then',
    ...
    '  exit 1',
    'fi',
    'if [ "$launcher_proof_required" = true ]; then launcher_nonce=$(od -An -N32 -tx1 /dev/urandom | tr -d \'[:space:]\'); fi',
    'if [ "$launcher_proof_required" = true ] && [ "${#launcher_nonce}" -ne 64 ]; then',
    ...
    '  exit 1',
    'fi',
    'if [ "$launcher_proof_required" = true ]; then',
    '  exec 9<<EOF',
