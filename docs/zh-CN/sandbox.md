...
控制时序集中在可注入的 policy 中：生产默认 control tick 为 250ms，慢速检查和容器 heartbeat 为 5s，parked 退避从 1s 增长到 5s，quiesce deadline 为 7s。测试可以注入更短的值，不需要改变安全状态机。新的 control root 使用 v5 manifest，保存精确容器 ID、受控 labels 和 root-relative `runtimeDir`。task-bound 容器会把它以读写方式挂载到 `/run/agent-infra/runtime`；客户端状态位于 `runtime/clients/<client>/<store>`，direct-host 则保留旧的 `.agents/workspace/.runtime/codex-*` fallback。创建前先取得 replacement lease、quiesce 旧 generation、确认精确旧容器 absent，之后才轮换 queue、processing evidence、runtime、token 和 generation 并启动新 broker。v5 broker 的正常路径不接受 v4；v4 只允许由显式 transition 流程读取。回滚到 v4 必须先用 v5 版本完整移除 v5 root 并确认 root absent，再由旧版本创建全新的 v4 root。transition 失败或结果未知时保留 control root 和证据。
