...
| `run-manual-validation` | 通过 snapshot 或 inplace 隔离运行宿主侧校验，并记录 current-only evidence envelope 和去敏证据。 | `task-id` 或 `--branch <ref>`、校验命令 | 校验真实容器、凭证或工作树行为，但不直接标记 PR 人工校验完成；任务工作区在另一台宿主时用 `--branch` 做跨环境校验。 |
...
| Skill | 描述 | 参数 | 推荐场景 |
|-------|------|------|---------|
| `create-pr` | 发布已审查的任务分支，并向任务绑定目标创建或复用 Pull Request。 | `task-id`（可选） | 已审查 checkpoint 的审查锚点和目标绑定仍有效时执行交付。 |
...
| Skill | 描述 | 参数 | 推荐场景 |
|-------|------|------|---------|
| `commit` | 创建直接 Git 提交，并附带任务状态更新和版权年份检查。 | 无 | 用于独立或无任务上下文的工作；常规任务生命周期由 `code-task` 创建 checkpoint。 |
