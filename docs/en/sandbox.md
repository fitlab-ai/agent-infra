...
Every sandbox has an explicit workspace identity. A task-bound sandbox mounts a private read-only workspace view and overlays only `.agents/workspace/active/<TASK-id>` as writable; tasks in every other state remain invisible. A branch-only sandbox receives the same stable directory structure with an empty short-id registry and no host task mount. The host workspace root is never mounted.
...
After upgrading from a release that mounted workspace states separately, the old container topology is intentionally treated as incompatible. Use the same explicit `--recreate` command once; container-local processes and writable-layer data may be lost, while the worktree and host-managed task data are preserved.
...
Control timing is centralized in an injectable policy: the production defaults are a 250 ms control tick, 5 s slow checks and container heartbeat, 1 s initial parked backoff growing to 5 s, and a 7 s quiesce deadline. Tests may supply shorter values without changing the safety state machine. New control roots use a v4 manifest with the exact container ID and controlled labels; creation inspects and atomically finalizes that identity before starting the broker. A pre-v4 control root or legacy PID record is never guessed, migrated, killed, or removed by the new version: rebuild the old sandbox/container with `--recreate`, and let the matching previous version handle legacy PID records.
...
Host-dependent checks use `ai task validate <branch | task-ref> [--scope snapshot|inplace] [--timeout <ms>] [--format text|json] -- <command>`. The default `snapshot` scope runs the command in a temporary detached worktree at the task branch commit and always removes it. `inplace` acquires a host-owned lease, waits for the broker to park, stops the sandbox container, runs against the original worktree, and then restores the branch, container, lease, and broker health. The `run-manual-validation` skill records sanitized `validation-run` evidence; `complete-manual-validation` remains the separate maintainer confirmation step.
