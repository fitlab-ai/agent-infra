...
Control timing is centralized in an injectable policy: the production defaults are a 250 ms control tick, 5 s slow checks and container heartbeat, 1 s initial parked backoff growing to 5 s, and a 7 s quiesce deadline. Tests may supply shorter values without changing the safety state machine. New control roots use a v5 manifest with the exact container ID, controlled labels, and a root-relative `runtimeDir`. Task-bound containers mount that directory read-write at `/run/agent-infra/runtime`; the client namespace is `runtime/clients/<client>/<store>`, while direct-host execution keeps its legacy `.agents/workspace/.runtime/codex-*` fallback. Creation first acquires a replacement lease, quiesces the old generation, confirms the exact old container is absent, and only then rotates queues, processing evidence, runtime, token, and generation before starting the new broker. A pre-v5 root is accepted only by this explicit transition path; the normal broker never runs a v4 manifest. A v5-to-v4 rollback must first remove the entire v5 root with the v5 binary, confirm it is absent, and then let the old version create a fresh v4 root. Failed or uncertain transitions retain the control root and evidence.
...
If in-place recovery fails, the command stops before entering the container or scheduling tmux. It does not replace the container automatically. For active and branch-only sandboxes, pass `--recreate` to authorize a container-only fallback: `ai sandbox start --recreate <target>`, `ai sandbox exec --recreate <target> [cmd...]`, or `ai run <skill> <task-ref> --recreate`. For `sandbox exec`, only a flag before the target is interpreted by the host; `--recreate` after the target is passed to the container command. Completed task-bound re-entry is the exception: readiness failure and `ai sandbox exec --recreate <branch>` are rejected without invoking replacement. The error gives a manual `docker exec` path and, if the user explicitly wants a new branch-only sandbox, the separate `ai sandbox rm <TASK-id>` and `ai sandbox create <branch>` commands. `ai sandbox rm` is the full interactive sandbox cleanup path, not a container-only delete; it may remove the worktree, local branch, tool/shell state, and branch share. Replacement preserves the worktree, local branch, host-managed tool seeds, shell configuration, and `/share` data, but discards the old container ID, writable layer, ordinary `/tmp`, processes, tmux sessions, and other RAM state. It never performs a full `ai sandbox rm`.
