...
Task-bound workspaces mount a host-created writable task projection. The authoritative task directory remains outside the container writable view. Workflow requests use a closed typed catalog and canonical top-level artifact basenames. For artifact landing, the host records the projection topology, opens the candidate with `O_NOFOLLOW`, reads and hashes it through the same descriptor, checks observable identity and metadata changes, and atomically writes the verified bytes to the authoritative directory. A replaced root, ancestor, symlink, digest, or artifact target fails closed. Projection edits never copy `task.md` or arbitrary files back into host task state; host workflow writers refresh only the protected task metadata they own.
